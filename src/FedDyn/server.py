...
class Server():
    ...
    def create_clients(self, local_datasets):
        ...
        for id_num,dataset in enumerate(local_datasets):
            client = Client(client_id=id_num, local_data=dataset, device=self.device, num_epochs = self.num_epochs, criterion = self.criterion, lr=self.lr_l, model = self.x)
    ...
    def server_update(self, client_ids):
        ...
        with torch.no_grad():
            ...
            #Update h
            for h, d_x in zip(self.h.parameters(), delta_x):
                h.data -= (self.alpha/self.num_clients) * d_x.data

            #Update x
            for x, a_y, h in zip(self.x.parameters(), avg_y, self.h.parameters()):
